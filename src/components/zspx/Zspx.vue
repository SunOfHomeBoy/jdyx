<template>
	<div class="zspx_wrap">
		<div class="zspx_content zspx_header">
			...
			<div class="zspx_main_contains">
				<!--<section v-for="(item,index) in list">-->
					...
					<pull-to :bottom-load-method="loadmore" @bottom-state-change="changebottomfn" :bottom-config="{pullText: '上拉加载',triggerText: '释放更新',loadingText: '加载中...',doneText: '加载完成',failText: '加载失败',loadedStayTime: 400,stayDistance: 50,triggerDistance: 70}" >
						<div>
							<div class="zspx_main_contains_img">
...
<script>
import { XHeader, GroupTitle, XButton, Divider, Grid, GridItem, Tabbar, TabbarItem, Group, Cell, XInput, Tab, TabItem, Scroller } from 'vux'
...
.zspx_wrap .vux-header{
	
	background:#2a7dad !important;
}
.zspx_wrap .vux-header .vux-header-title{
...
div.zspx_main_contains{
	height:90%;
	
}
div.zspx_main_contains ul{
	padding-left:.2rem;
	margin-top:.2rem;
}
div.zspx_main_contains ul li{
	border-bottom:1px solid #bbbbbb;
	padding:.3rem 0;
}
div.zspx_main_contains ul li:not(:first-child){
	margin-top:.1rem;
}
div.zspx_main_contains ul li dl{
	display:flex;
}
div.zspx_main_contains ul li dt{
	width:2rem;
	height:2rem;
	display:flex;
	align-items:center;
}
div.zspx_main_contains ul li dt img{
	display:block;
}
div.zspx_main_contains ul li dd{
	display:flex;
	flex-direction:column;
	font-size:0.23rem;
	color:#bbbbbb;
...
div.zspx_main_contains ul li dd h4{
	font-size:.3rem;
	color:#414141;
	
}
div.zspx_main_contains_img{
	width:100%;
	
}
div.zspx_main_contains_img img{
	width:100%;
